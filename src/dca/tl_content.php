 ...
 */
if (Input::get('do') == 'projects')
{
	$GLOBALS['TL_DCA']['tl_content']['config']['ptable'] = 'tl_portfolio_project';
	//$GLOBALS['TL_DCA']['tl_content']['config']['onload_callback'][] = array('tl_content_projects', 'checkPermission');
	//$GLOBALS['TL_DCA']['tl_content']['list']['operations']['toggle']['button_callback'] = array('tl_content_projects', 'toggleIcon');
}
 ...
 */
class tl_content_projects extends Backend
{
	...
	public function checkPermission()
	{
		...
		switch (Input::get('act'))
		{
			...
			case 'copyAll':
				...
				$objCes = $this->Database->prepare("SELECT id FROM tl_content WHERE ptable='tl_project' AND pid=?")
										 ->execute(CURRENT_ID);
	...
	protected function checkAccessToElement($id, $root, $blnIsPid=false)
	{
		if ($blnIsPid)
		{
			$objArchive = $this->Database->prepare("SELECT a.id, n.id AS nid FROM tl_project n, tl_project_category a WHERE n.id=? AND n.pid=a.id")
										 ->limit(1)
										 ->execute($id);
		}
		else
		{
			$objArchive = $this->Database->prepare("SELECT a.id, n.id AS nid FROM tl_content c, tl_project n, tl_project_category a WHERE c.id=? AND c.pid=n.id AND n.pid=a.id")
										 ->limit(1)
										 ->execute($id);
		}
		...
		// The project category is not mounted
		if (!in_array($objArchive->id, $root))
		{
			$this->log('Not enough permissions to modify article ID ' . $objArchive->nid . ' in project category ID ' . $objArchive->id, __METHOD__, TL_ERROR);
